<?php
    ...
    if (isset($_GET['action']) && $_GET['action'] === 'get_available_timeslots' && isset($_GET['date'])) {
        $date = $_GET['date'];
    ...
    // Get booked timeslots for this date
    $query = "SELECT time FROM time_slot WHERE date = ?";
    $stmt = mysqli_prepare($con, $query);
    mysqli_stmt_bind_param($stmt, 's', $date);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    ...
    $available_timeslots = [];
    foreach ($all_timeslots as $time => $display) {
        if (!in_array($time, $booked_times)) {
            $available_timeslots[$time] = $display;
        }
    }
...
if (isset($_GET['date']) && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    ...
    if (isset($_GET['action']) && $_GET['action'] === 'get_timeslots') {
        ...
        $events = array();
        while ($row = mysqli_fetch_assoc($result)) {
            $startTime = date("H:i", strtotime($row['time']));
            $endTime = date("H:i", strtotime($row['time']) + 3600);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['date']) && isset($_POST['time'])) {
        ...
        $insertQuery = "INSERT INTO time_slot (date, time, no_driver_per_slot, admin_id) VALUES (?, ?, ?, ?)";
        $stmt = $con->prepare($insertQuery);
        $stmt->bind_param("ssii", $date, $time, $driverCount, $admin_id);
...
<style>
    ...
    * {
    ...
    input[type="date"]:disabled::-webkit-datetime-edit-fields-wrapper {
        color: #999;
    }
    ...
    .modal-close:hover, 
    button[onclick="closeSlotModal()"]:hover, 
    button[onclick="closeViewSlotModal()"]:hover {
        color: #333 !important;
    }
    ...
    #deleteTimeslotBtn {
        background-color: #c6433a !important;
        color: white;
        cursor: pointer;
    }

    #deleteTimeslotBtn:disabled {
        background-color: #d3a7a7 !important;
        cursor: not-allowed !important;
    }

    #deleteTimeslotBtn:not(:disabled):hover {
        background-color: #b53a32 !important;
    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    eventContent: function(arg) {
        return {
            html: `<div style="font-size:0.95em; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${arg.event.title}</div>`
        };
    },
    ...
    events: function(fetchInfo, successCallback, failureCallback) {
        ...
        .then(events => {
            ...
            setTimeout(adjustCellHeights, 100);
        })
        .catch(error => {
...
document.getElementById('pickupDate').addEventListener('change', function() {
    ...
    fetch(`Admin-PickupAvailability.php?action=get_available_timeslots&date=${date}`)
        .then(response => response.json())
        .then(availableTimeslots => {
            const timeSelect = document.getElementById('selectedSlotInput');
            timeSelect.innerHTML = '<option value="">-- Select a time slot --</option>';
            for (const [time, display] of Object.entries(availableTimeslots)) {
            ...
            if (Object.keys(availableTimeslots).length === 0) {
                timeSelect.innerHTML = '<option value="">No available timeslots for this date</option>';
            }
        })
        .catch(error => {
    ...
    // Style the native date picker's disabled dates
...
function adjustCellHeights() {
    const eventContainers = document.querySelectorAll('.fc-daygrid-day-events');
    ...
    calendar.render();
    setTimeout(adjustCellHeights, 100);
    window.addEventListener('resize', () => setTimeout(adjustCellHeights, 100));
    ...
    slotDateSelect.addEventListener('change', function() {
    if (this.value) {
        const [day, month, year] = this.value.split('/');
        const apiDate = `${year}-${month}-${day}`;
        loadTimeslotsForDate(apiDate);
    } else {
        // Clear timeslot dropdown when no date is selected
    ...
    fetch("Admin-PickupAvailability.php?action=get_timeslot_dates")
        ...
        .then(datesStr => {
            ...
            // Clear timeslot dropdown initially
            timeslotSelect.innerHTML = '<option value="">-- Select a time slot --</option>';
    ...
    deleteTimeslotBtn.style.backgroundColor = '#c6433a'; 
    ...
    if (selectedOption && selectedOption.value) {
        ...
        if (bookingCount > 0) {
            ...
            deleteTimeslotBtn.style.backgroundColor = '#d3a7a7'; 
    ...
    if (!formData.get('time') || formData.get('time') === "") {
    ...
    .finally(() => {
        ...
        submitBtn.textContent = 'Add Pickup Slot';
    });
});
    ...
    if (!slotDateSelect.value) {
        slotDateSelect.setCustomValidity('Please select a date.');
        slotDateSelect.reportValidity();
        return;
    } else {
        slotDateSelect.setCustomValidity('');
    }
    
    // Validate timeslot selection
    if (!slotId || slotId === "") {
        timeslotSelect.setCustomValidity('Please select a time slot from the list.');
        timeslotSelect.reportValidity();
        return;
    } else {
        timeslotSelect.setCustomValidity('');
    }
    
    if (selectedOption && parseInt(selectedOption.getAttribute('data-booking-count')) > 0) {
        alert('Cannot delete timeslot with existing bookings');
        return;
    }
...
<body>
    <div class="sidebar">
        ...
        <ul class="menu">
            ...
            <li><a href="Admin-Review.php?page-nr=1"><i class="fa-solid fa-comments"></i>Review</a></li>
...
<!-- Calendar -->
        <section class="calendar">
            <div id="fullCalendar" style="width: 86%; margin-left:83px; background-color: white; padding:40px 40px 10px; position: relative; border-radius: 10px 10px 0 0;"></div>
            <div class="legend-container" style="width: 86%; margin-left: 83px; background-color: white; padding-top: 10px; padding-bottom: 40px; border-radius: 0 0 10px 10px; display: flex; justify-content: center; gap: 20px; margin-top: -1px;">
                ...
                <div class="legend-item">
                    <div class="legend-color" style="background-color: rgb(237, 203, 203); border: solid 1px;"></div>
        ...
        <button onclick="closeSlotModal()" style="position: absolute; right: 40px; top: 40px; background: none; border: none; font-size: 35px; cursor: pointer; color: #666;">&times;</button>
        
        <h2 style="margin-bottom: 15px; font-size: 30px; color: black; font-weight: bold; text-align: left;">Add New Pickup Slot</h2>
                
                <p style="margin-bottom: 38px; color: #595959; font-size: 16px; line-height: 1.5;">Please fill in this form to add a new pickup slot.</p>
                ...
                <div style="margin-bottom: 30px;">
                    ...
                    <input type="date" name="date" id="pickupDate" required 
                        style="width: 100%; padding: 12px; border-radius: 5px; border: 1px solid #ddd; font-size: 16px; font-family: Arial, sans-serif;"
                    ...
                    <div style="margin-bottom: 30px;">
                        ...
                        <div class="custom-select">
                            <select name="time" id="selectedSlotInput" class="select-with-arrow" required style="width: 100%; padding: 10px 48px 10px 12px;; border-radius: 5px; border: 1px solid #ddd; font-size: 16px; color: #333;" oninvalid="this.setCustomValidity('Please select a time slot from the list.')"
                    ...
                    <div style="margin-top: auto; margin-bottom: 10px; display: flex; justify-content: center;">
                        <button type="submit" style="background: #4E7831; color: white; padding: 12px 0; border: none; border-radius: 5px; cursor: pointer; font-size: 16px; width: 100%; max-width: 600px;">
                            Add Pickup Slot
                        </button>
                    </div>
                </form>
    ...
    <aside id="viewSlotModal" style="display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.5); backdrop-filter: blur(5px); z-index: 9999; justify-content: center; align-items: center;">
        <div class="modal-content">
            <div class="containerz">
                ...
                <h2 style="margin-bottom: 15px; text-align: left; font-size: 30px; color: black;">Pickup Slots</h2>
                
                <p style="font-family: Arial, sans-serif; font-size: 16px; font-weight: normal; color: #595959; margin-bottom: 38px;">
                ...
                <div style="margin-bottom: 25px;">
                    ...
                    <div class="custom-select">
                        <select id="slotDateSelect" class="select-with-arrow" required 
                            style="width: 100%; padding: 15px; font-size: 16px; border-radius: 5px; border: 1px solid #ccc; height: 50px;"
                ...
                <div style="margin-bottom: 25px;">
                    ...
                    <div class="custom-select">
                    <select id="timeslotSelect" class="select-with-arrow" required style="width: 100%; padding: 15px; font-size: 16px; border-radius: 5px; border: 1px solid #ccc; height: 50px;">
                        <option value="">-- Select a time slot --</option>
                    </select>
                    </div>
                </div>
