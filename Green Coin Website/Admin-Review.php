<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['review_id'])) { 
        ...
        $user_id = mysqli_fetch_assoc(mysqli_query($conn, 
        "SELECT dr.user_id AS 'user_id' FROM review r INNER JOIN dropoff dr ON r.dropoff_id = dr.dropoff_id WHERE r.review_id = '$reviewId'
        UNION ALL 
        SELECT pr.user_id AS 'user_id' FROM review r INNER JOIN pickup_request pr ON r.pickup_request_id = pr.pickup_request_id 
        WHERE r.review_id = '$reviewId'"))['user_id'];
        
        // Check if this review already has a reply to prevent duplicates
        $checkQuery = "SELECT * FROM reply_review WHERE review_id = ?";
        ...
        if ($result->num_rows > 0) {
            // Already has a reply, just redirect
        ...
        if ($reviewId && $replyText) {
            ...
            $query = "INSERT INTO reply_review (review_id, review, date, admin_id) VALUES (?, ?, ?, ?)";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("issi", $reviewId, $replyText, $date, $admin_id);

            if ($stmt->execute()) {
                // Add a success parameter to the redirect
                $announcement = "Your review has received a response! 📝✨ Visit the Review page to read the reply and stay connected with our team. Thank you for sharing your feedback! 💚";
                $announcement = mysqli_real_escape_string($conn, $announcement);
                $notificationQuery = "INSERT INTO user_notification(user_id, datetime, title, announcement, status, admin_id) 
                                    VALUES ('$user_id', NOW(), 'Your Review Got a Reply! 💬', '$announcement', 'unread', '$admin_id')";
...
<body>
    <div class="sidebar">
            ...
            <li class="active"><a href="Admin-Review.php"><i class="fa-solid fa-comments"></i>Review</a></li>
            ...
            <section class="review-container">
                <?php 
                        ...
                        echo '<div class="review-row">';
                            ...
                            // Only show reply form if there's no existing reply
                            if (!$hasReply) {
                                $hasContent = isset($_POST['reply_text']) && $_POST['reply_text'] !== '' && 
                                             isset($_POST['review_id']) && $_POST['review_id'] == $reviewID;
                                
                                echo '<div class="reply-form-container' . ($hasContent ? ' active' : '') . '" id="reply-form-'.$reviewID.'">';
                                echo '<form class="reply-form" data-review-id="'.$reviewID.'">';
                                echo '<textarea class="reply-textarea" name="reply_text" placeholder="Write your reply here...">';
                                if ($hasContent) {
                                    echo htmlspecialchars($_POST['reply_text']);
                                }
                                echo '</textarea>';
                                echo '<div style="text-align: right; padding-right: 15px;">';
                                echo '<button type="submit" class="submit-reply-btn"><i class="material-icons" style="font-size:20px;">&#xe163;</i></button>';
                                echo '</div>';
                                echo '</form>';
                                echo '</div>';
                            }
...
<script>
    ...
    document.querySelectorAll('.reply-form-container').forEach(container => {
        ...
        if (textarea && textarea.value.trim() !== '') {
            ...
            submitBtn.style.marginBottom = "0px";
        }
